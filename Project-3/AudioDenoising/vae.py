...
class VAE:
    """
    Represents a Deep Convolutional Variational Autoencoder.
    """
    ...
    def _add_bottleneck(self, x):
        ...
        x = Lambda(normal_distribution, name='encoder_output')(
    ...
    def _calculate_combined_loss(self, y_target, y_predicted):
        return self.reconstruction_loss_weight * mse(y_target, y_predicted) + \
               self._calculate_kl_loss(y_target, y_predicted)
    ...
    def _calculate_kl_loss(self, y_target, y_predicted):
        kl_loss = - 0.5 * K.sum(1 + self.log_variance - K.square(self.mu) - K.exp(self.log_variance),
                                axis=-1)
        return kl_loss
